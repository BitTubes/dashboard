(function() {
	...
	function MonitoringController(http, $scope, $rootScope, Auth, $locale) {
		...
		function drawChart(data) {
			...
			var numberPattern = '#' + $locale.NUMBER_FORMATS.DECIMAL_SEP + '###' + $locale.NUMBER_FORMATS.GROUP_SEP;
			var msPattern = numberPattern + '### ms';
			...
			vm.myChartObject.options = {
				...
				legend: { position: 'top' },
				vAxes: {
					0: {title: 'Response Time (ms)', minValue: 0, format: msPattern },
					1: {title: 'Error Rate (errors/call)', minValue: 0, maxValue: 0.05, format: numberPattern + '## %'}
				},
				series: {
					0: { targetAxisIndex: 0 },
					1: { targetAxisIndex: 0 },
