(function() {
	...
	function configController(http, $scope, $rootScope, $uibModal, _, note, Auth) {
		...
		function initView() {
			...
			http.post($scope.uriApiCms+'getConfig', { 'api': $rootScope.DEFAULT_API, p:{'ID': config_ID} })
			.then(function(response){
				// var data = response.data;
			...
			.then(function(response){
				...
				for (var i = data.length - 1; i >= 0; i--) {
					makeBackup(data[i]);
					data['localeName'] = vm.availableLocales[data['locale']];
					delete vm.availableLocales[data['locale']];
				}
